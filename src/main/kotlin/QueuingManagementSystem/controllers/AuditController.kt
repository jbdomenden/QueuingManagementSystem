...
class AuditController {
    fun createAuditLog(actorUserId: Int?, departmentId: Int?, action: String, entityName: String, entityId: String, payloadJson: String?): Boolean {
        QueuingManagementSystem.config.ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
            QueuingManagementSystem.queries.postAuditLogQuery
    ...
    fun getAuditLogs(departmentId: Int?): MutableList<QueuingManagementSystem.models.AuditLogModel> { val list = mutableListOf<QueuingManagementSystem.models.AuditLogModel>(); QueuingManagementSystem.config.ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(if (departmentId == null) QueuingManagementSystem.queries.getAuditLogsQuery else QueuingManagementSystem.queries.getAuditLogsByDepartmentQuery).use { s -> if (departmentId != null) s.setInt(1, departmentId); s.executeQuery().use { rs -> while (rs.next()) list.add(
        QueuingManagementSystem.models.AuditLogModel(
            rs.getInt("id"),
