...
import QueuingManagementSystem.queries.getQueueTypesByDepartmentQuery
import QueuingManagementSystem.queries.postQueueTypeQuery
import QueuingManagementSystem.queries.updateQueueTypeQuery
import kotlin.compareTo

class QueueTypeController {
    fun createQueueType(request: QueueTypeRequest): Int { ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
        postQueueTypeQuery
    ...
    fun updateQueueType(request: QueueTypeRequest): Boolean { ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
        updateQueueTypeQuery
    ).use { s -> s.setString(1, request.name); s.setString(2, request.code); s.setString(3, request.prefix); s.setBoolean(4, request.is_active); s.setInt(5, request.id ?: 0); return s.executeUpdate() compareTo 0 } } }
    fun getQueueTypesByDepartment(departmentId: Int): MutableList<QueueTypeModel> { val list = mutableListOf<QueueTypeModel>(); ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
        getQueueTypesByDepartmentQuery
