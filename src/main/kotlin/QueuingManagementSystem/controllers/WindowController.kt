...
class WindowController {
    fun createWindow(request: QueuingManagementSystem.models.WindowRequest): Int { QueuingManagementSystem.config.ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
        QueuingManagementSystem.queries.postWindowQuery
    ...
    fun updateWindow(request: QueuingManagementSystem.models.WindowRequest): Boolean { QueuingManagementSystem.config.ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
        QueuingManagementSystem.queries.updateWindowQuery
    ...
    fun getWindowsByDepartment(departmentId: Int): MutableList<QueuingManagementSystem.models.WindowModel> { val list = mutableListOf<QueuingManagementSystem.models.WindowModel>(); QueuingManagementSystem.config.ConnectionPoolManager.getConnection().use { c -> c.prepareStatement(
        QueuingManagementSystem.queries.getWindowsByDepartmentQuery
    ).use { s -> s.setInt(1, departmentId); s.executeQuery().use { rs -> while (rs.next()) list.add(
        QueuingManagementSystem.models.WindowModel(
            rs.getInt("id"),
