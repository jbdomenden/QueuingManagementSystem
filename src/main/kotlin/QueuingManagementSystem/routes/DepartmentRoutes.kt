    ...
    route("/departments") {
        post("/create") {
            try {
                ...
                if (session.role != UserRole.SUPERADMIN.name) return@post call.respond(HttpStatusCode.Forbidden,
                ...
                call.respond(HttpStatusCode.OK,
                    IdResponse(
                        ...
                        GlobalCredentialResponse(200, true, "Department created")
                    )
                )
        ...
        put("/update") {
            try {
                val session = authController.getUserSessionByToken(call.request.extractBearerToken())
                if (session.role != UserRole.SUPERADMIN.name) return@put call.respond(HttpStatusCode.Forbidden,
                    GlobalCredentialResponse(403, false, "Forbidden")
                )
                val request = call.receive<DepartmentRequest>()
                val errors = request.validateDepartmentRequest()
                if (errors.isNotEmpty() || request.id == null) return@put call.respond(HttpStatusCode.BadRequest, errors.ifEmpty { mutableListOf(
                    GlobalCredentialResponse(400, false, "id is required")
                ) })
                call.respond(HttpStatusCode.OK,
                    GlobalCredentialResponse(200, departmentController.updateDepartment(request), "Department updated")
                )
            } catch (e: Exception) { call.respond(HttpStatusCode.InternalServerError,
                GlobalCredentialResponse(500, false, e.message ?: "Internal server error")
            ) }
        ...
        get("/list") {
            try {
                ...
                call.respond(HttpStatusCode.OK,
                    ListResponse(departmentController.getDepartments(), GlobalCredentialResponse(200, true, "OK"))
                )
            } catch (e: Exception) { call.respond(HttpStatusCode.InternalServerError,
                GlobalCredentialResponse(500, false, e.message ?: "Internal server error")
            ) }
