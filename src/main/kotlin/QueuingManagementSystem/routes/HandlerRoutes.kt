...
fun Route.handlerRoutes() {
    ...
    val controller = QueuingManagementSystem.controllers.HandlerController()
    route("/handlers") {
        post("/create") { try { val session = authController.getUserSessionByToken(call.request.extractBearerToken()); if (session.role !in listOf(
            ...
            QueuingManagementSystem.models.IdResponse(
                ...
                QueuingManagementSystem.models.GlobalCredentialResponse(200, true, "Handler created")
            )
        ...
        post("/start-session") { try { val session = authController.getUserSessionByToken(call.request.extractBearerToken()); if (session.role !in listOf(
                QueuingManagementSystem.common.UserRole.HANDLER.name, QueuingManagementSystem.common.UserRole.SUPERADMIN.name, QueuingManagementSystem.common.UserRole.DEPARTMENT_ADMIN.name)) return@post call.respond(HttpStatusCode.Forbidden,
            QueuingManagementSystem.models.GlobalCredentialResponse(403, false, "Forbidden")
